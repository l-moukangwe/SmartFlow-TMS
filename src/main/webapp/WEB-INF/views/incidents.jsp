...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="/smartflow/incidents">Incidents</a></li>
                ...
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link" href="/smartflow/logout">Logout</a></li>
                </ul>
            </div>
        </div>
    </nav>
    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-exclamation-triangle"></i> Incidents Management</h2>
                    ...
                    <table class="table table-striped table-hover">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th>Status</th><th>Reported By</th><th>Reported At</th><th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:forEach items="${incidents}" var="incident">
                                <tr>
                                    ...
                                    <td><span class="badge bg-${incident.severity == 'Critical' ? 'danger' : (incident.severity == 'High' ? 'warning' : 'secondary')}">${incident.severity}</span></td>
                                    <td>${incident.location}</td>
                                    <td><span class="badge bg-${incident.status == 'Active' ? 'danger' : 'success'}">${incident.status}</span></td>
                                    <td>${incident.reported_by_name}</td>
                                    <td>${incident.reported_at}</td>
                                    <td>
                                        <c:if test="${incident.status == 'Active'}">
                                            <a href="/smartflow/incidents/resolve?id=${incident.incident_id}" class="btn btn-sm btn-success" onclick="return confirm('Resolve this incident?')">
